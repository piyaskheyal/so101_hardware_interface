...
It is recommended to use a Python virtual environment with system site packages enabled (so ROS 2 core packages remain accessible) to install the necessary dependencies for the Feetech servos.

```bash
# Navigate to your workspace root
cd ~/ws_ros2/ws_moveit2

# Create the virtual environment with ROS 2 system packages access
...
pip install -r src/so101_hardware_interface/requirements.txt
```
...
```bash
# Run the calibration tool as a standalone script (no need to source workspace)
python3 src/so101_hardware_interface/scripts/calibrate.py
```
